...
export function ContributorsSection({
  ...
  // Excludes the current profile so a user cannot add themselves as a contributor.
  const getBuilderProfiles = useCallback((): ProfileOption[] => {
    ...
    const profileOptions: ProfileOption[] = builderProfileNodesWithDriveId
      .filter(({ node }) => node.id !== currentProfileId)
      .map(({ node }) => {
    ...
    for (const remoteProfile of remoteProfiles) {
      if (remoteProfile.id === currentProfileId) continue;
